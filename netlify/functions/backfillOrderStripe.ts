...
import {runOrderStripeBackfill} from '../lib/backfills/orderStripe'
  ...
  const limit =
    typeof limitRaw === 'number'
      ...
      : typeof limitRaw === 'string'
        ? Number(limitRaw)
        : undefined
  const kindRaw = (payload.kind ?? event.queryStringParameters?.kind ?? '').toString().toLowerCase()

  if (!['checkout', 'paymentintent', 'payment_intent', 'charge'].includes(kindRaw)) {
    return {
      statusCode: 400,
      headers: {...CORS, 'Content-Type': 'application/json'},
      body: JSON.stringify({error: 'Missing or invalid kind (checkout|paymentIntent|charge)'}),
    }
  }
    ...
    const result = await runOrderStripeBackfill({
      ...
      id:
        typeof payload.id === 'string'
          ? payload.id
          : typeof event.queryStringParameters?.id === 'string'
            ? event.queryStringParameters?.id
            : undefined,
      kind: normalizeKind(kindRaw) as 'checkout' | 'paymentIntent' | 'charge',
    })
