  ...
  try {
    if (SYNC_SECRET) {
      const providedSecret =
        event.headers['x-merchant-sync-secret'] || event.headers['X-MERCHANT-SYNC-SECRET']
    ...
    products.forEach((product: any, index: number) => {
      try {
        ...
        if (!offerId || !price || !link || !imageLink) {
          skipped.push({
            id: product?._id || offerId || `index-${index}`,
            reason: 'Missing offerId, price, link, or image.',
          })
          return
        }
        ...
        const condition = (product?.condition || product?.productCondition || 'new')
          .toString()
          .toLowerCase()
        ...
        const googleProductCategory =
          product?.googleProductCategory || product?.google_product_category
        const shippingLabel =
          product?.shippingLabel || (product?.installOnly ? 'install_only' : undefined)
        const productHighlights = Array.isArray(derivedFeed?.highlights)
          ? derivedFeed.highlights
          : []
        ...
        skipped.push({
          id: product?._id || `index-${index}`,
          reason: err?.message || 'Failed to build product payload',
        })
      }
    })
