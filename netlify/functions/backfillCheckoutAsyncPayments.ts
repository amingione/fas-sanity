...
import {runCheckoutAsyncPaymentsBackfill} from '../lib/backfills/checkoutAsyncPayments'
  ...
  const limit =
    typeof limitRaw === 'number'
      ...
      : typeof limitRaw === 'string'
        ? Number(limitRaw)
        : undefined
  const statusRaw = (payload.status ?? event.queryStringParameters?.status) as
    | 'all'
    | 'success'
    | 'failure'
    | undefined
  const normalizedStatus =
    statusRaw && ['all', 'success', 'failure'].includes(statusRaw)
      ? (statusRaw as 'all' | 'success' | 'failure')
      : undefined

  try {
    const result = await runCheckoutAsyncPaymentsBackfill({
      dryRun,
