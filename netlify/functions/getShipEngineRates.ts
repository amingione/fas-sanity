...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const isShipToUS = typeof ship_to.country_code === 'string' && ship_to.country_code.toUpperCase() === 'US'
    const isShipFromUS = typeof ship_from.country_code === 'string' && ship_from.country_code.toUpperCase() === 'US'
    ...
    if (missingFields.length > 0) {
      return {
        statusCode: 400,
        headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: `Missing required fields for ShipEngine: ${missingFields.join(', ')}` }),
      }
    }
...
if (!rates || rates.length === 0) {
  ...
  if (!haveToFull || !haveFromFull) {
    ...
    const from_state = shipFromIsUS ? toUsAbbr(ship_from.state_province) : ship_from?.state_province
...
return {
  statusCode: 200,
  headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' },
  body: JSON.stringify({ rates, debug: { usedFallback: (!rates || rates.length === 0) ? false : !Array.isArray((data as any)?.rate_estimates) } }),
}
  } catch (error: any) {
    return {
      statusCode: 500,
