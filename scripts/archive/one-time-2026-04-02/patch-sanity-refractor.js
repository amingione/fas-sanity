...
function main() {
  ...
  try {
    ...
    console.warn(
      '[patch-sanity-refractor] failed to guard worker file:',
  ...
  try {
    ...
    console.warn(
      '[patch-sanity-refractor] failed to normalize @sanity/diff:',
      err && err.message ? err.message : err,
    )
  }
  ...
  try {
    ...
    console.warn(
      '[patch-sanity-refractor] failed to clamp react-refractor:',
      err && err.message ? err.message : err,
    )
  }
  ...
  try {
    ...
    if (fs.existsSync(uiEs)) {
      ...
      s = s.replace(
        /t0\s*=\s*language\s*\?\s*Refractor\.hasLanguage\(language\)\s*:\s*!1/g,
        't0 = !1',
      )
      // And common compiled variant where default wrapper is used
      s = s.replace(
        /t0\s*=\s*language\s*\?\s*Refractor__default\.default\.hasLanguage\(language\)\s*:\s*!1/g,
        't0 = !1',
      )
      // Fallback: if the full ternary still exists within the conditional sequence, collapse it explicitly
      s = s.replace(
        /\$\[0\]\s*!==\s*language\s*\?\s*\(t0\s*=\s*language\s*\?[^:]+:\s*!1\s*,/g,
        '$[0] !== language ? (t0 = !1,',
      )
      fs.writeFileSync(uiEs, s)
      console.log(
        '[patch-sanity-refractor] patched @sanity/ui ESM refractor to disable highlighting',
      )
    }
    ...
    if (fs.existsSync(uiCjs)) {
      ...
      console.log(
        '[patch-sanity-refractor] patched @sanity/ui CJS refractor to disable highlighting',
      )
    }
  } catch (err) {
    console.warn(
      '[patch-sanity-refractor] failed to patch @sanity/ui refractor chunk:',
      err && err.message ? err.message : err,
    )
  }
  ...
  try {
    ...
    console.warn(
      '[patch-sanity-refractor] failed to patch @sanity/ui Toast bundles:',
      err && err.message ? err.message : err,
    )
  }
}
...
function patchToastDistFile(filePath, {variant}) {
  ...
  const alreadyForwardRef =
    /\bconst Toast\s*=\s*forwardRef/.test(source) ||
    /\bconst Toast\s*=\s*react\.forwardRef/.test(source)
  ...
  source = source.replace(
    /function Toast\(\s*props\)/,
    `const Toast = ${refFactory}(function Toast(props, forwardedRef)`,
  )
  ...
  source = source.replace(
    'exit, transition, children:',
    'exit, transition, ref: forwardedRef, children:',
  )
