...
if (!projectId || !dataset || !token) {
  console.error('Missing SANITY envs')
  process.exit(1)
}
...
async function run() {
  ...
  while (true) {
    ...
    for (const d of docs) {
      ...
      if (Object.keys(setOps).length || !d.updatedAt) {
  ...
  console.log(
    JSON.stringify({total, changed, optInDefaults, updatedStamped, rolesDefaulted}, null, 2),
  )
}
