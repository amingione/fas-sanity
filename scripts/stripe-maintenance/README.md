...
| `backfill-missing-fields.js` | Queries orders that are missing card details, receipt URLs, invoice references, or customer links and backfills them from Stripe. Also patches the related customer with billing address + Stripe ID. Limit via `BACKFILL_LIMIT` (default 50). | `node scripts/stripe-maintenance/backfill-missing-fields.js` |
...
3. **Backfill missing Sanity fields**  
   ...
   ```
   Populates `cardBrand`, `cardLast4`, `receiptUrl`, `invoiceRef`, and customer billing metadata.
