  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement('label', null,
        React.createElement('input', { type: 'radio', checked: mode === 'product', onChange: () => { setMode('product'); onChange(set({ ...(value || {}), kind: 'product' })) } }),
        ' ', 'Product'
      ),
      React.createElement('label', null,
        React.createElement('input', { type: 'radio', checked: mode === 'custom', onChange: () => { setMode('custom'); onChange(set({ ...(value || {}), kind: 'custom', product: undefined })) } }),
        ' ', 'Custom'
      ),
    ),
    ...
    mode === 'product'
      ? React.createElement(
              ...
              ? React.createElement(
                  ...
                  ...results.map((p) =>
                    React.createElement(
                      ...
                      React.createElement('div', { style: { fontSize: 12, color: '#444' } }, p.sku || '—'),
                      React.createElement('div', { style: { fontSize: 12 } }, `$${Number(p.price || 0).toFixed(2)}`),
                    )
                  )
                )
              : null,
          ...
          React.createElement(
            ...
            React.createElement('div', { style: { fontSize: 12, textAlign: 'right' } }, `= $${(computed || 0).toFixed(2)}`)
          )
        )
      : null,
