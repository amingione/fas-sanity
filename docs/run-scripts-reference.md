...
| Expired checkout tagging           | `pnpm tsx scripts/backfill-expired-checkouts.ts`                                                               | Replays Stripe expiration events so `abandonedCheckout` docs stay in sync (may need multiple runs).                                          |
