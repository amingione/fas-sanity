...
All items must be signed off by the designated role. **No partial passes — every gate must be green.**
...
| Role | Responsibility |
|------|----------------|
| **Platform Lead** | Overall architecture integrity; authority boundary enforcement; CI/governance gates |
| **Commerce Lead** | Pricing, order, and payment pipeline; Medusa authority boundary; Stripe/Shippo flows |
| **Ops Lead** | Fulfillment queue, returns/refunds, reconciliation, tracking lifecycle |
| **Content Lead** | Sanity schema compliance; editorial workflow health; content/commerce separation |

Each gate row lists the required sign-off role in the **Sign-Off** column. Fill in initials and date on release day (e.g., `[✓ AL 2026-04-08]`).
...
| Check | Repo | Pass Criteria | Sign-Off (Platform Lead) |
|-------|------|---------------|--------------------------|
...
Sign-off owner: **Commerce Lead**

| Check | Repo | Pass Criteria | Sign-Off (Commerce Lead) |
|-------|------|---------------|--------------------------|
...
Sign-off owners: **Commerce Lead** (WS3-1, WS3-2) · **Ops Lead** (WS3-3)

| Check | Repo | Pass Criteria | Sign-Off |
|-------|------|---------------|----------|
| Checkout path: storefront → Medusa → Stripe via Medusa | fas-cms-fresh, fas-medusa | No `stripe.paymentIntents.create` outside Medusa payment module | `[ ]` Commerce Lead |
| Stripe webhook idempotency | fas-medusa | `src/api/webhooks/stripe/route.ts` has pre-check SELECT + post-failure duplicate guard | `[ ]` Commerce Lead |
| Reconciliation queue surfaced in Dash | fas-dash | `/orders/reconciliation` page exists and filters `requires_manual_reconciliation: true` from order metadata | `[ ]` Ops Lead |
| Ghost order filter on all fulfillment surfaces | fas-dash | Pending fulfillment counts require `paymentStatus ∈ {captured, paid}` AND `status ∉ {canceled}` on all three surfaces (FulfillmentTableClient, orders/page, fulfillment/page) | `[ ]` Ops Lead |
| Order total resolution priority chain | fas-dash | `order-customer-mappers.ts` resolves total via: `metadata.total_cents` → `metadata.legacy_total_amount` → Medusa `total` (with shipping-only guard) | `[ ]` Ops Lead |
...
| Check | Repo | Pass Criteria | Sign-Off (Ops Lead) |
|-------|------|---------------|---------------------|
...
Sign-off owner: **Content Lead**

| Check | Repo | Pass Criteria | Sign-Off (Content Lead) |
|-------|------|---------------|-------------------------|
...
Perform these in order after all gates are green. Each step has an assigned owner.

| Step | Action | Owner |
|------|--------|-------|
| 1 | Run `npm run typecheck && npm run lint` in fas-dash — must exit 0 | Platform Lead |
| 2 | Merge main branch to `production` (or equivalent) in each repo | Platform Lead |
| 3 | Confirm Vercel deployment succeeds for fas-dash (fasmotorsports.io admin) | Ops Lead |
| 4 | Confirm Netlify deployment succeeds for fas-cms-fresh (fasmotorsports.io storefront) | Commerce Lead |
| 5 | Smoke test: place a test order end-to-end (storefront → Medusa → Stripe capture → fulfillment queue → tracking) | Commerce Lead + Ops Lead |
| 6 | Verify reconciliation queue shows 0 unresolved events | Ops Lead |
| 7 | Verify fulfillment queue shows only captured/paid, non-cancelled orders | Ops Lead |
| 8 | Tag release in GitHub with format `v{YYYY.MM.DD}-pipeline-{gate}` (e.g. `v2026.04.08-pipeline-gate6`) | Platform Lead |

---

## Rollback Triggers

Initiate rollback if any of the following occur post-promotion. The owner listed is responsible for triage and initiating rollback.

| Trigger | Owner |
|---------|-------|
| Payment intents created outside Medusa (authority boundary violation) | Commerce Lead |
| Sanity price fields written from application code (authority violation) | Content Lead |
| Fulfillment queue shows ghost/expired orders (filter regression) | Ops Lead |
| Order totals display incorrect amounts (mapper regression) | Ops Lead |
| Stripe webhook creates duplicate orders for same `payment_intent` (idempotency failure) | Commerce Lead |

Rollback procedure: revert to previous Vercel/Netlify deployment; open GitHub issue with WS tracker ID reference; notify Platform Lead.
