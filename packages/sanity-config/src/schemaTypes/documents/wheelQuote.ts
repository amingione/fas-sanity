...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Backspacing',
      type: 'string',
      description: 'e.g., 2.25" (skinny), 7.5 in, S550 Mustang',
    }),
    defineField({
      name: 'finish',
      title: 'Finish',
    ...
    defineField({
      ...
      title: 'Beadlock',
      type: 'string',
      options: {list: ['None', 'Single Beadlock', 'Double Beadlock']},
    }),
    defineField({
      name: 'hardware',
      title: 'Hardware',
      type: 'string',
      options: {
        list: ['Standard ARP', 'Upgraded ARP (color)', 'Black hardware', 'Polished hardware'],
      },
    }),
    defineField({
      name: 'centerCap',
      title: 'Center Cap',
      type: 'string',
      options: {list: ['Standard', 'Black', 'Polished', 'Color-matched']},
    }),
  ...
  preview: {
    ...
    prepare(sel) {
      const title = sel.title
        ? `${sel.title} — ${sel.series} ${sel.diameter}x${sel.width}`
        : `${sel.series} ${sel.diameter}x${sel.width}`
