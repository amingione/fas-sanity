...
export const invoiceLineItemType = defineType({
  ...
  fields: [
    defineField({ name: 'kind', title: 'Kind', type: 'string', initialValue: 'product', options: { list: ['product','custom'], layout: 'radio' } }),
  ...
  preview: {
    ...
    prepare({ title, qty, price, kind, sku }: any) {
      ...
      return { title: `${label}${sku ? ` • ${sku}`:''} — ${qty||1} × $${Number(price||0).toFixed(2)}` }
    },
  },
})
