...
export default React.forwardRef<HTMLDivElement, Record<string, never>>(
  function EnvSelfCheck(_props, ref) {
    ...
    return (
      <div ref={ref} style={{padding: 16, color: '#000'}}>
        ...
        {err && !loading && <div style={{color: '#c00'}}>Error: {err}</div>}
        {!loading && !err && data && (
          <div>
            <Section
              title="Sanity"
            ...
            {!data.ok && (
              <div style={{color: '#b26b00', marginTop: 12}}>
                Some required variables are missing. Update Site settings → Environment variables in
