...
import {AddIcon, CheckmarkIcon, CloseIcon, FilterIcon, SearchIcon} from '@sanity/icons'
...
function normalizeProduct(raw: RawProduct): ProductRow {
  ...
  const totalInventorySource: 'manual' | 'automatic' | null =
    manualInventory !== null ? 'manual' : totalInventory !== null ? 'automatic' : null
  ...
  const variantBasePrice = variantInventory.prices.length
    ? Math.min(...variantInventory.prices)
    : null
  ...
  const inventoryValue =
    basePrice !== null && totalInventory !== null ? basePrice * totalInventory : null
...
function computeMetrics(rows: ProductRow[]): Metrics {
  ...
  const lowOrOutCount = rows.filter(
    (row) => row.inventoryStatus === 'low' || row.inventoryStatus === 'out',
  ).length
...
const ProductListDashboard = forwardRef<HTMLDivElement | null, Record<string, never>>(
  function ProductListDashboard(_props, ref) {
    ...
    const filteredProducts = useMemo(() => {
      ...
      return normalizedProducts.filter((product) => {
        ...
        if (vendorFilters.size > 0 && (!product.vendor || !vendorFilters.has(product.vendor)))
          return false
        ...
        if (inventoryFilters.size > 0 && !inventoryFilters.has(product.inventoryStatus))
          return false
        return true
      })
        ...
        try {
          ...
          setError(
            (err as Error)?.message ||
              'Unable to load products. Please try again or check your connection.',
          )
        } finally {
    ...
    const handleImportFile = useCallback(
      async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
          ...
          const idIndex = normalizedHeaders.findIndex(
            (header) => header === 'sanity id' || header === '_id' || header === 'id',
          )
          const skuIndex = normalizedHeaders.findIndex((header) => header === 'sku')
          const installIndex = normalizedHeaders.findIndex(
            (header) => header === 'install only' || header === 'install_only',
          )
          ...
          if (installIndex === -1) {
            toast.push({
              ...
              description:
                'Include an "Install Only" column with TRUE or FALSE values to apply updates.',
            })
            return
          }
          ...
          setRawProducts((prev) =>
            prev.map((product) =>
              updates.has(product._id)
                ? {...product, installOnly: updates.get(product._id)}
                : product,
            ),
          )
    ...
    const handleSetInstallOnly = useCallback(
      async (value: boolean) => {
        ...
        try {
          ...
          setRawProducts((prev) =>
            prev.map((product) =>
              selectedIds.has(product._id) ? {...product, installOnly: value} : product,
            ),
          )
          toast.push({
            status: 'success',
    ...
    const handleSaveView = useCallback(() => {
      ...
      toast.push({
        status: 'success',
        title: 'View saved',
        description: `Saved filters as "${name}".`,
      })
    ...
    return (
      <Flex ref={ref} direction="column" height="fill" style={{background: '#f8fafc'}}>
        ...
        <Box padding={5} style={{flex: '1 1 auto', overflow: 'auto'}}>
          <Stack space={5}>
            <Flex align="flex-start" justify="space-between" gap={4} style={{flexWrap: 'wrap'}}>
              <Stack space={3} style={{minWidth: 240}}>
                ...
                <Text muted size={1}>
                  Monitor inventory health, sales readiness, and merchandising coverage across all
                  channels.
                </Text>
              </Stack>
            ...
            >
              <Stack space={4}>
                <Flex align="center" justify="space-between" gap={3} style={{flexWrap: 'wrap'}}>
                  <Flex align="center" gap={3} style={{flexWrap: 'wrap', flex: '1 1 480px'}}>
                    ...
                    <MenuButton
                      ...
                      menu={
                        <Menu>
                          {(Object.keys(inventoryFilterLabels) as InventoryStatus[]).map(
                            (status) => (
                              <MenuItem
                                key={status}
                    ...
                    <Stack space={2}>
                      <Flex align="center" justify="space-between">
                        ...
                        <Select
                          value={timeframe}
                          onChange={(event) =>
                            setTimeframe(event.currentTarget.value as typeof timeframe)
                          }
                          style={{maxWidth: 120}}
                        >
                      ...
                      >
                        ...
                        >
                          <Checkbox
                            checked={isSelected}
                            onChange={(event) =>
                              handleSelect(product.id, event.currentTarget.checked)
                            }
                          />
                        </span>
                        <span
