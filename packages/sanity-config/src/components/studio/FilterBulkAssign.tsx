  ...
  const performLoad = useCallback(
    async (term: string) => {
      ...
      try {
        ...
        const query = `*[_type == "product" && !( $tag in (defined(filters) ? filters : []) ) ${trimmed ? ' && (title match $m || sku match $m)' : ''}][0...200]{ _id, title, sku }`
      ...
      <div style={{maxHeight: 380, overflowY: 'auto', border: '1px solid #eee', borderRadius: 6}}>
        {items.length === 0 ? (
          <div style={{padding: 10, color: '#777'}}>
            No matching products (or all already have “{normTag}”).
          </div>
        ) : (
          items.map((p) => (
            <label
